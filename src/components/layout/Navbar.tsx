...
export function Navbar() {
  ...
  /** Transparent chrome only on dark heroes at top; mobile drawer forces solid. */
  const transparent =
  ...
  return (
    ...
    >
      <div className="container-page relative z-50 flex h-14 items-center justify-between gap-3 sm:h-16 md:h-[4.25rem]">
        ...
        <nav
          className={cn(
            "absolute left-1/2 top-1/2 hidden -translate-x-1/2 -translate-y-1/2 items-center gap-8 text-2xl xl:flex xl:gap-11",
              ...
              return (
                <li key={l.label}>
                  <Link
                    to={l.to}
                    onClick={() => setOpen(false)}
                    className="group flex min-h-14 w-full items-center justify-between gap-4 border-b border-border/60 py-3 text-left transition-colors active:bg-muted/50"
                  >
                    <span className="flex items-baseline gap-3">
                      <span className="font-display text-xs font-medium tracking-wide text-muted-foreground/55">
                        {index}
                      </span>
                      <span className="font-display text-xl font-normal tracking-tight transition-colors group-active:text-primary sm:text-2xl">
                        {l.label}
                      </span>
                    </span>
                    <FiArrowUpRight
