  ...
  return (
    <div className="mt-8 space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="border-border/70 p-5 sm:p-6">
          ...
          <ul className="mt-4 space-y-4">
            {cohorts.map((c) => {
              ...
              const pct = Math.min(100, Math.round((filled / c.capacity) * 100));
              return (
                <li key={c.id}>
                  <div className="mb-1.5 flex justify-between gap-2 text-sm">
                    <span className="truncate font-medium">{c.name}</span>
                    <span className="shrink-0 text-muted-foreground">
                      {filled}/{c.capacity} · {waiting} waiting
                    </span>
                  </div>
            ...
            {cohorts.length === 0 && (
              <p className="text-base text-muted-foreground">No cohorts yet.</p>
            )}
          </ul>
        </Card>
        )}
        ...
        {showFinancing && (
        <Card className="border-border/70 p-5 sm:p-6">
          <p className="text-eyebrow text-muted-foreground">Financing readiness</p>
        ...
        {role === "bank_partner" && (
          <Card className="border-border/70 p-5 sm:p-6">
            <p className="text-eyebrow text-muted-foreground">Deposit readiness</p>
            <div className="mt-4 space-y-4">
              <HorizontalBar
                label="Average deposit available"
                value={avgDeposit}
      ...
      <Card className="border-border/70 p-5 sm:p-6">
        <div className="flex flex-wrap items-end justify-between gap-4">
          ...
          <DonutChart
            ...
            segments={[
              ...
              {
                value: candidates.filter((c) => c.has_bank_account && c.needs_uza_access_support)
                  .length,
                color: "var(--volt)",
                label: "Both",
              },
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-3">
          {[
            { label: "Bank account", value: banked, total: candidates.length, className: "[&>div]:bg-primary" },
            { label: "UZA Access requested", value: accessSupport, total: candidates.length, className: "[&>div]:bg-destructive" },
            { label: "No bank account", value: candidates.length - banked, total: candidates.length, className: "[&>div]:bg-muted-foreground/50" },
          ].map((item) => (
